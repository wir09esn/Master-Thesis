...
def obj_rule(m):
    return sum(m.x_Total[t] * m.P[t] + (m.x_PV_to_Home[t]  + m.x_in_PV_EV[t] + m.x_PV_in_storage[t]+ m.x_PV_in_Grid[t]) * m.COST_PV[t]  - m.x_PV_in_Grid[t] * m.P_PV[t] for t in m.T)
...
data = pd.DataFrame({   ' x_total_from_Grid ' : [instance.x_Total[t].value for t in instance.T],
                        ...
                        ' PV Production ' : [instance.PV[t] for t in instance.T]})
#print(data)
data.to_excel('M6_Home_PV_EV_Storage.xlsx', sheet_name = 'Sheet1')
